...
2. Запустите `initialization.sh` для создания демона, который будет использовать бота. Управлять состоянием сервиса можно с помощью `systemctl`.
3. После первого запуска программа создаст файл `config.ini` в корневой директории.
4. В файле `config.ini` необходимо указать следующие данные:
...
5. После настройки `config.ini` перезапустите приложение.
6. Если бот больше не нужен используйте `stop_and_remove_service.sh`, затем очистите исполняемую директорию бота.
...
- **Удалить файл чатов**: Удаляет файл со всеми id чатов, в которых используется бот.
...
3. Перейдите в Telegram и откройте чат с ботом.
